...
This is `ditto_nh`, the native NostrHost (`_nh`) package for
[Ditto](https://ditto.pub) ([source](https://gitlab.com/soapbox-pub/ditto)),
scaffolded from
...
below: `[app].id` = `ditto`, `UPSTREAM_REPO` =
`https://gitlab.com/soapbox-pub/ditto.git`, `PACKAGE_ID` = `ditto`,
`OUTPUT_DIR` = `dist`, `BUILD_ENV` empty (Ditto builds with its own
hardcoded defaults when no `ditto.json` is supplied — see `build.yml`'s
...
broke `ditto`'s route and both self-healed after these fixes.
